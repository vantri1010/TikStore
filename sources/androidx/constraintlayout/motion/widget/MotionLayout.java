...
public class MotionLayout extends ConstraintLayout implements NestedScrollingParent3 {
    ...
    public void setupMotionViews() {
        ...
        if (stagger != 0.0f) {
            if (((double) stagger) >= FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE) {
                flip = false;
            }
        ...
        private void debugLayout(String title, ConstraintWidgetContainer c) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (v instanceof TextView) {
                    name = name + SQLBuilder.PARENTHESES_LEFT + ((TextView) v).getText() + SQLBuilder.PARENTHESES_RIGHT;
                }
                ...
                if (motionController != null) {
                    if (this.mStart != null) {
                        ...
                        if (startWidget != null) {
                            ...
                            Log.e(MotionLayout.TAG, Debug.getLocation() + "no widget for  " + Debug.getName(v2) + " (" + v2.getClass().getName() + SQLBuilder.PARENTHESES_RIGHT);
                        }
                    }
                    if (this.mEnd != null) {
        ...
        public void draw(Canvas canvas, HashMap<View, MotionController> frameArrayList, int duration, int debugPath) {
            if (frameArrayList != null && frameArrayList.size() != 0) {
                ...
                if (!MotionLayout.this.isInEditMode() && (debugPath & 1) == 2) {
                    String str = MotionLayout.this.getContext().getResources().getResourceName(MotionLayout.this.mEndState) + LogUtils.COLON + MotionLayout.this.getProgress();
    ...
    private void checkStructure(int csetId, ConstraintSet set) {
        ...
        for (int i2 = 0; i2 < ids.length; i2++) {
            ...
            if (set.getHeight(id2) == -1) {
                Log.w(TAG, "CHECK: " + setName + SQLBuilder.PARENTHESES_LEFT + idString + ") no LAYOUT_HEIGHT");
            }
