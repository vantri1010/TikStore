...
public class ConstraintLayoutStates {
        ...
        public State(Context context, XmlPullParser parser) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (attr == R.styleable.State_android_id) {
                    ...
                    if (TtmlNode.TAG_LAYOUT.equals(type)) {
    ...
    private void parseConstraintSet(Context context, XmlPullParser parser) {
        ...
        for (int i = 0; i < count; i++) {
            if (TtmlNode.ATTR_ID.equals(parser.getAttributeName(i))) {
                String s = parser.getAttributeValue(i);
                int id = -1;
                if (s.contains("/")) {
                    id = context.getResources().getIdentifier(s.substring(s.indexOf(47) + 1), TtmlNode.ATTR_ID, context.getPackageName());
                }
                if (id == -1) {
