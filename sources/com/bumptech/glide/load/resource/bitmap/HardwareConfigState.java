...
public final class HardwareConfigState {
    ...
    public boolean isHardwareConfigAllowed(int targetWidth, int targetHeight, boolean isHardwareConfigAllowed, boolean isExifOrientationRequired) {
        if (!isHardwareConfigAllowed || !this.isHardwareConfigAllowedByDeviceModel || Build.VERSION.SDK_INT < 26 || isExifOrientationRequired || targetWidth < minHardwareDimension || targetHeight < minHardwareDimension || !isFdSizeBelowHardwareLimit()) {
            return false;
        }
        return true;
    }
    ...
    private synchronized boolean isFdSizeBelowHardwareLimit() {
        ...
        if (i >= 50) {
            ...
            if (!z && Log.isLoggable("Downsampler", 5)) {
                Log.w("Downsampler", "Excluding HARDWARE bitmap config because we're over the file descriptor limit, file descriptors " + currentFds + ", limit " + fdSizeLimit);
            }
        }
