...
public final class Ac3Util {
    ...
    private static final int[] BITRATE_BY_HALF_FRMSIZECOD = {32, 40, 48, 56, 64, 80, 96, 112, 128, 160, PsExtractor.AUDIO_STREAM, 224, 256, 320, 384, 448, 512, 576, UVCCamera.DEFAULT_PREVIEW_WIDTH};
    ...
    public static int parseAc3SyncframeSize(byte[] data) {
        ...
        if (!(((data[5] & UByte.MAX_VALUE) >> 3) == 16)) {
    ...
    public static int parseTrueHdSyncframeAudioSampleCount(byte[] syncframe) {
        ...
        if ((syncframe[7] & UByte.MAX_VALUE) == 187) {
            isMlp = true;
        }
    ...
    public static int parseTrueHdSyncframeAudioSampleCount(ByteBuffer buffer, int offset) {
        return 40 << ((buffer.get((buffer.position() + offset) + ((buffer.get((buffer.position() + offset) + 7) & UByte.MAX_VALUE) == 187 ? 9 : 8)) >> 4) & 7);
    }
