...
final class DecodedBitStreamParser {
    ...
    private static int byteCompaction(int mode, int[] codewords, Charset encoding, int codeIndex, StringBuilder result) {
        ...
        if (i == BYTE_COMPACTION_MODE_LATCH) {
            ...
            if (codeIndex3 == codewords[0] && nextCode < TEXT_COMPACTION_MODE_LATCH) {
                byteCompactedCodewords[codeIndex2] = nextCode;
                codeIndex2++;
            }
            for (int i2 = 0; i2 < codeIndex2; i2++) {
    ...
    private static int numericCompaction(int[] codewords, int code, StringBuilder result) throws FormatException {
        ...
        while (code < codewords[0] && !end) {
            ...
            if (codeIndex2 < TEXT_COMPACTION_MODE_LATCH) {
                ...
                if (!(codeIndex2 == TEXT_COMPACTION_MODE_LATCH || codeIndex2 == BYTE_COMPACTION_MODE_LATCH || codeIndex2 == 928)) {
                    switch (codeIndex2) {
                        case MACRO_PDF417_TERMINATOR /*922*/:
                        case BEGIN_MACRO_PDF417_OPTIONAL_FIELD /*923*/:
                        case BYTE_COMPACTION_MODE_LATCH_6 /*924*/:
