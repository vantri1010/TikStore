...
public class ChangeTimeScaleTrack implements Track {
    ...
    static long[] adjustTts(long[] sourceArray, double timeScaleFactor, long[] syncSample, long[] syncSampleTimes) {
        long[] jArr = sourceArray;
        long summedDurations = 0;
        long[] scaledArray = new long[jArr.length];
        for (int i = 1; i <= jArr.length; i++) {
            long x = Math.round(((double) jArr[i - 1]) * timeScaleFactor);
            int binarySearch = Arrays.binarySearch(syncSample, (long) (i + 1));
            int ssIndex = binarySearch;
            if (binarySearch >= 0 && syncSampleTimes[ssIndex] != summedDurations) {
    ...
    public String getName() {
        return "timeScale(" + this.source.getName() + SQLBuilder.PARENTHESES_RIGHT;
    }
