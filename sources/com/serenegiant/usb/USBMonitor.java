...
public final class USBMonitor {
    ...
    private final Runnable mDeviceCheckRunnable = new Runnable() {
        public void run() {
            ...
            if (!USBMonitor.this.destroyed) {
                ...
                USBMonitor.this.mAsyncHandler.postDelayed(this, AdaptiveTrackSelection.DEFAULT_MIN_TIME_BETWEEN_BUFFER_REEVALUTATION_MS);
            }
        }
    };
    /* access modifiers changed from: private */
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (!USBMonitor.this.destroyed) {
                ...
                if (USBMonitor.this.ACTION_USB_PERMISSION.equals(action)) {
                    ...
                    boolean unused = uSBMonitor.updatePermission(device3, uSBMonitor.hasPermission(device3));
    ...
    public USBMonitor(Context context, OnDeviceConnectListener listener) {
        if (listener != null) {
            ...
            this.mAsyncHandler = BaseHandler.createHandler(TAG);
    ...
    public final void dumpDevices() {
        ...
        if (list != null) {
            ...
            for (String key : keys) {
                ...
                Log.i(TAG, "key=" + key + LogUtils.COLON + device + LogUtils.COLON + sb.toString());
            }
            return;
        }
        Log.i(TAG, "no device");
    ...
    public static final String getDeviceKeyName(UsbDevice device, String serial, boolean useNewAPI) {
        ...
        if (useNewAPI && BaseBuildCheck.isAndroid5()) {
            sb.append("#");
            if (TextUtils.isEmpty(serial)) {
    ...
    public static UsbDeviceInfo updateDeviceInfo(UsbManager manager, UsbDevice device, UsbDeviceInfo _info, UsbDeviceConnection connection) {
        byte[] desc;
        ...
        if (device != null) {
            ...
            if (!(manager == null || !manager.hasPermission(device) || usbDeviceConnection == null || (desc = connection.getRawDescriptors()) == null || desc.length < 17)) {
                if (TextUtils.isEmpty(info.usb_version)) {
                    info.usb_version = String.format("%x.%02x", new Object[]{Integer.valueOf(desc[3] & UByte.MAX_VALUE), Integer.valueOf(desc[2] & UByte.MAX_VALUE)});
                }
                if (TextUtils.isEmpty(info.version)) {
                    info.version = String.format("%x.%02x", new Object[]{Integer.valueOf(desc[13] & UByte.MAX_VALUE), Integer.valueOf(desc[12] & UByte.MAX_VALUE)});
                }
                ...
                try {
                    ...
                    if (languageCount > 0) {
                        if (TextUtils.isEmpty(info.manufacturer)) {
                            info.manufacturer = getString(usbDeviceConnection, desc[14], languageCount, languages);
                        }
                        if (TextUtils.isEmpty(info.product)) {
                            info.product = getString(usbDeviceConnection, desc[15], languageCount, languages);
                        }
                        if (TextUtils.isEmpty(info.serial)) {
                            info.serial = getString(usbDeviceConnection, desc[16], languageCount, languages);
                        }
                    }
                } finally {
                    connection.close();
    ...
    public static final class UsbControlBlock implements Cloneable {
        ...
        private UsbControlBlock(USBMonitor monitor, UsbDevice device) {
            ...
            Log.e(USBMonitor.TAG, "openDevice");
            String[] strArr = null;
            this.mInfo = USBMonitor.updateDeviceInfo(monitor.mUsbManager, device, (UsbDeviceInfo) null, this.mConnection);
            String name = device.getDeviceName();
            String[] v = !TextUtils.isEmpty(name) ? name.split("/") : strArr;
            int busnum = 0;
            int devnum = 0;
            if (v != null) {
            ...
            if (usbDeviceConnection != null) {
                ...
                Log.i(USBMonitor.TAG, String.format(Locale.US, "name=%s,desc=%d,busnum=%d,devnum=%d,rawDesc=", new Object[]{name, Integer.valueOf(desc), Integer.valueOf(busnum), Integer.valueOf(devnum)}) + rawDesc);
                return;
            }
