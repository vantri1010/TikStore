...
public class StatsController extends BaseController {
    ...
    private int bytesToInt(byte[] bytes) {
        return (bytes[0] << 24) | ((bytes[1] & UByte.MAX_VALUE) << 16) | ((bytes[2] & UByte.MAX_VALUE) << 8) | (bytes[3] & UByte.MAX_VALUE);
    }
    ...
    private StatsController(int account) {
        ...
        if (needConvert) {
            ...
            for (int i2 = 3; a2 < i2; i2 = 3) {
                ...
                jArr[a2] = sharedPreferences.getLong("resetStatsDate" + a2, 0);
                for (int b2 = 0; b2 < 7; b2++) {
                    long[] jArr2 = this.sentBytes[a2];
                    jArr2[b2] = sharedPreferences.getLong("sentBytes" + a2 + "_" + b2, 0);
                    long[] jArr3 = this.receivedBytes[a2];
                    jArr3[b2] = sharedPreferences.getLong("receivedBytes" + a2 + "_" + b2, 0);
                ...
                long[] jArr4 = this.resetStatsDate;
                if (jArr4[a2] == 0) {
                    save2 = true;
                    jArr4[a2] = System.currentTimeMillis();
                }
                a2++;
            }
            if (save2) {
    ...
    private void saveStats() {
        long newTime = System.currentTimeMillis();
        if (Math.abs(newTime - lastStatsSaveTime.get().longValue()) >= AdaptiveTrackSelection.DEFAULT_MIN_TIME_BETWEEN_BUFFER_REEVALUTATION_MS) {
