...
public class ID3v2Info extends AudioInfo {
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (LOGGER.isLoggable(debugLevel2)) {
                                ...
                                logger.log(debugLevel2, "ID3 exception occured: " + e2.getMessage());
                            }
                        } catch (Throwable th) {
                ...
                if (trck.length() > 0) {
                    ...
                    if (index < 0) {
                        try {
                            ...
                            if (LOGGER.isLoggable(this.debugLevel)) {
                                LOGGER.log(this.debugLevel, "Could not parse disc number: " + trck);
                                return;
                            }
                            return;
                        }
                        ...
                        try {
                            ...
                            if (LOGGER.isLoggable(this.debugLevel)) {
                                LOGGER.log(this.debugLevel, "Could not parse number of discs: " + trck);
                                return;
                            }
                            return;
                        }
                ...
                if (trck2.length() > 0) {
                    ...
                    if (index2 < 0) {
                        try {
                            ...
                            if (LOGGER.isLoggable(this.debugLevel)) {
                                LOGGER.log(this.debugLevel, "Could not parse track number: " + trck2);
                                return;
                            }
                            return;
                        }
                        ...
                        try {
                            ...
                            if (LOGGER.isLoggable(this.debugLevel)) {
                                LOGGER.log(this.debugLevel, "Could not parse number of tracks: " + trck2);
                                return;
                            }
                            return;
                        }
    ...
    public CommentOrUnsynchronizedLyrics parseCommentOrUnsynchronizedLyricsFrame(ID3v2FrameBody data) throws IOException, ID3v2Exception {
        ...
        return new CommentOrUnsynchronizedLyrics(data.readFixedLengthString(3, ID3v2Encoding.ISO_8859_1), data.readZeroTerminatedString(ItemTouchHelper.Callback.DEFAULT_DRAG_ANIMATION_DURATION, encoding), data.readFixedLengthString((int) data.getRemainingLength(), encoding));
    }
    ...
    public AttachedPicture parseAttachedPictureFrame(ID3v2FrameBody data) throws IOException, ID3v2Exception {
        ...
        return new AttachedPicture(data.getData().readByte(), data.readZeroTerminatedString(ItemTouchHelper.Callback.DEFAULT_DRAG_ANIMATION_DURATION, encoding), imageType, data.getData().readFully((int) data.getRemainingLength()));
    }
}
