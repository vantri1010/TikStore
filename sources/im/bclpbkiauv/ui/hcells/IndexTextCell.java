...
public class IndexTextCell extends FrameLayout {
    private AvatarDrawable avatarDrawable;
    private SimpleTextView countText;
    private View fc_point_root;
    private ImageView imageView;
    private BackupImageView iv_head;
    ...
    private View view_fc_point;

    public IndexTextCell(Context context) {
        this(context, 23);
    }

    public IndexTextCell(Context context, int left) {
        super(context);
        ...
        BackupImageView backupImageView = (BackupImageView) this.pointView.findViewById(R.id.iv_head);
        this.iv_head = backupImageView;
        backupImageView.setRoundRadius(AndroidUtilities.dp(5.0f));
        this.view_fc_point = this.pointView.findViewById(R.id.tv_fc_point);
    ...
    public void setShowPointView(TLRPC.User itemUser, boolean isShow) {
        if (!isShow || itemUser == null) {
            this.iv_head.setVisibility(4);
            this.fc_point_root.setVisibility(8);
            return;
        }
        ...
        this.fc_point_root.setVisibility(0);
    }
    ...
    public void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (LocaleController.isRTL) {
            int dp = AndroidUtilities.dp((float) this.leftPadding);
        }
        ...
        if (this.imageView.getVisibility() == 0) {
            int viewTop2 = (height - AndroidUtilities.dp(36.5f)) / 2;
            int viewLeft2 = AndroidUtilities.dp(15.0f);
            this.imageView.layout(viewLeft2, viewTop2, AndroidUtilities.dp(36.5f) + viewLeft2, AndroidUtilities.dp(36.5f) + viewTop2);
        }
        if (this.pointView.getVisibility() == 0) {
    ...
    public void setTextAndIcon(String text, String imageLeftUrl, int padding, int rightId, boolean divider) {
        this.textView.setText(text);
        this.valueTextView.setText((CharSequence) null);
        GlideUtils.getInstance().load(imageLeftUrl, getContext(), this.imageView, (int) R.mipmap.img_emtpy_error_default, (int) R.mipmap.img_empty_default);
        this.imageView.setVisibility(0);
        this.valueTextView.setVisibility(8);
        this.valueImageView.setVisibility(0);
        this.valueImageView.setImageResource(rightId);
