...
import im.bclpbkiauv.ui.hui.friendscircle_v1.view.FCIndexBar;
...
public class ChooseAtContactsActivity extends BaseSearchViewFragment implements NotificationCenter.NotificationCenterDelegate {
        ...
        this.fragmentView = new FrameLayout(context) {
            ...
            public void onLayout(boolean changed, int left, int top, int right, int bottom) {
                ...
                if (ChooseAtContactsActivity.this.listView.getAdapter() != ChooseAtContactsActivity.this.listViewAdapter) {
                    ChooseAtContactsActivity.this.emptyView.setTranslationY((float) AndroidUtilities.dp(0.0f));
                } else if (ChooseAtContactsActivity.this.emptyView.getVisibility() == 0) {
                    ChooseAtContactsActivity.this.emptyView.setTranslationY((float) AndroidUtilities.dp(74.0f));
                }
            }
        };
    ...
    private void initList(FrameLayout frameLayout, Context context) {
        ...
        this.searchListViewAdapter = new SearchAdapter(context, (SparseArray<TLRPC.User>) null, this.allowUsernameSearch, false, false, this.allowBots, true, 0);
        AnonymousClass2 r0 = new CreateSecureAdapter(context) {
    ...
    private void initSideBar(FrameLayout frameLayout, Context context) {
        ...
        this.sideBar.setOnTouchingLetterChangedListener(new FCIndexBar.OnTouchingLetterChangedListener() {
    ...
    public AnimatorSet onCustomTransitionAnimation(boolean isOpen, final Runnable callback) {
        ...
        this.set.setDuration(200);
        if (isOpen) {
            this.set.playTogether(new Animator[]{ObjectAnimator.ofFloat(this.actionBar, View.TRANSLATION_Y, new float[]{(float) parentHeight, 0.0f}), ObjectAnimator.ofFloat(this.actionBar, View.ALPHA, new float[]{0.0f, 1.0f}), ObjectAnimator.ofFloat(this.fragmentView, View.TRANSLATION_Y, new float[]{(float) (parentHeight + actionBarHeight), 0.0f}), ObjectAnimator.ofFloat(this.fragmentView, View.ALPHA, new float[]{0.0f, 1.0f})});
        } else {
            this.set.playTogether(new Animator[]{ObjectAnimator.ofFloat(this.actionBar, View.TRANSLATION_Y, new float[]{0.0f, (float) parentHeight}), ObjectAnimator.ofFloat(this.actionBar, View.ALPHA, new float[]{1.0f, 0.0f}), ObjectAnimator.ofFloat(this.fragmentView, View.TRANSLATION_Y, new float[]{0.0f, (float) (parentHeight + actionBarHeight)}), ObjectAnimator.ofFloat(this.fragmentView, View.ALPHA, new float[]{1.0f, 0.0f})});
        }
