...
public class PagerGridLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    public void updateAttrs(int rows, int cloumns) {
        this.mRows = rows;
        this.mColumns = cloumns;
        this.mOnePageSize = rows * cloumns;
    }
    ...
    public int getPages() {
        return getItemCount() / this.mOnePageSize;
    }
    ...
    private void recycleAndFillItems(RecyclerView.Recycler recycler, RecyclerView.State state, boolean isStart) {
        if (!state.isPreLayout()) {
            ...
            int startPos2 = startPos - (this.mOnePageSize * 2);
            if (startPos2 < 0) {
                startPos2 = 0;
            }
            int stopPos = (this.mOnePageSize * 4) + startPos2;
            ...
            if (isStart) {
                for (int i = startPos2; i < stopPos; i++) {
                    addOrRemove(recycler, displayRect, i);
                }
            } else {
                for (int i2 = stopPos - 1; i2 >= startPos2; i2--) {
    ...
    private void setPageIndex(int pageIndex, boolean isScrolling) {
        ...
        PagerConfig.Loge("setPageIndex = " + pageIndex + LogUtils.COLON + isScrolling);
    ...
    public int setOrientationType(int orientation) {
        ...
        this.mOffsetY = (x / getUsableWidth()) * getUsableHeight();
    ...
    public void scrollToPage(int pageIndex) {
        ...
        if (pageIndex < 0 || pageIndex >= this.mLastPageCount) {
            String str = TAG;
            Log.e(str, "pageIndex = " + pageIndex + " is out of bounds, mast in [0, " + this.mLastPageCount + SQLBuilder.PARENTHESES_RIGHT);
        } else if (this.mRecyclerView == null) {
            Log.e(TAG, "RecyclerView Not Found!");
        } else {
            if (canScrollVertically()) {
