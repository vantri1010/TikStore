...
class AndroidLoggerFactory implements ILoggerFactory {
    ...
    static String loggerNameToTag(String loggerName) {
        ...
        do {
            int indexOf = loggerName.indexOf(46, lastTokenIndex);
            int lastPeriodIndex = indexOf;
            if (indexOf != -1) {
    ...
    private static String getSimpleName(String loggerName) {
        ...
        if (lastPeriodIndex != -1 && length - (lastPeriodIndex + 1) <= 23) {
            return loggerName.substring(lastPeriodIndex + 1);
        }
        return '*' + loggerName.substring((length - 23) + 1);
    }
}
